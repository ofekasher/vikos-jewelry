    ...
    <>
      ...
      >
        <AnimatePresence mode="wait">
          {open ? (
            <motion.svg key="x" initial={{ rotate: -90, opacity: 0 }} animate={{ rotate: 0, opacity: 1 }} exit={{ rotate: 90, opacity: 0 }} transition={{ duration: 0.2 }} width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="#C9A96E" strokeWidth="1.5">
              <line x1="18" y1="6" x2="6" y2="18"/><line x1="6" y1="6" x2="18" y2="18"/>
            </motion.svg>
          ) : (
            <motion.svg key="ai" initial={{ scale: 0.7, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} exit={{ scale: 0.7, opacity: 0 }} transition={{ duration: 0.2 }} width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="#C9A96E" strokeWidth="1.4">
              <path d="M12 2a4 4 0 0 1 4 4v1h1a3 3 0 0 1 0 6h-1v1a4 4 0 0 1-8 0v-1H7a3 3 0 0 1 0-6h1V6a4 4 0 0 1 4-4z"/>
              <circle cx="9" cy="9" r="1" fill="#C9A96E"/><circle cx="15" cy="9" r="1" fill="#C9A96E"/>
            </motion.svg>
          )}
        </AnimatePresence>
        {hasNew && (
          <span style={{ position: "absolute", top: 0, right: 0, width: "10px", height: "10px", borderRadius: "50%", background: "#C9A96E", border: "2px solid #111" }} />
        )}
      </motion.button>
      ...
      <AnimatePresence>
        {open && (
          ...
          >
            {/* Header */}
            <div style={{ padding: "16px 20px", borderBottom: "1px solid rgba(201,169,110,0.15)", display: "flex", alignItems: "center", gap: "12px" }}>
              ...
              <div>
                ...
                <p style={{ fontFamily: "'Inter',sans-serif", fontSize: "9px", color: "rgba(201,169,110,0.7)", margin: 0, letterSpacing: "0.1em", textTransform: "uppercase" }}>AI · זמינה עכשיו</p>
              </div>
            </div>
            ...
            <div style={{ flex: 1, overflowY: "auto", padding: "16px", display: "flex", flexDirection: "column", gap: "12px" }}>
              ...
              <div style={{ alignSelf: "flex-start", maxWidth: "80%" }}>
                <div style={{ background: "rgba(201,169,110,0.1)", border: "1px solid rgba(201,169,110,0.2)", borderRadius: "2px 12px 12px 12px", padding: "10px 14px" }}>
              ...
              {msgs.map((m, i) => (
                <div key={i} style={{ alignSelf: m.role === "user" ? "flex-end" : "flex-start", maxWidth: "80%" }}>
                  <div style={{
                    background: m.role === "user" ? "#C9A96E" : "rgba(255,255,255,0.05)",
              ...
              {loading && (
                <div style={{ alignSelf: "flex-start" }}>
                  <div style={{ background: "rgba(255,255,255,0.05)", border: "1px solid rgba(255,255,255,0.08)", borderRadius: "2px 12px 12px 12px", padding: "12px 16px", display: "flex", gap: "5px", alignItems: "center" }}>
                    {[0, 1, 2].map(i => (
                      <span key={i} style={{ width: "6px", height: "6px", borderRadius: "50%", background: "#C9A96E", animation: `bounce 1.2s ease-in-out ${i * 0.2}s infinite` }} />
                    ))}
                  </div>
                </div>
              )}
